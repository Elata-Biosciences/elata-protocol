...
contract Deploy is Script {
    ...
    address public ADMIN_MSIG =
        vm.envAddress("ADMIN_MSIG");
    address public INITIAL_TREASURY = vm
        .envAddress("INITIAL_TREASURY");

    // Token configuration
    uint256 public constant
        INITIAL_MINT = 10_000_000 ether; // 10M ELTA initial
        ...
        )
    {
        ...
        protocol.staking = new VeELTA(
            protocol.token, ADMIN_MSIG
        );
        console2.log(
            "   VeELTA deployed at:",
        ...
        protocol.governor = new ElataGovernor(
            protocol.token
        );
        console2.log(
        ...
        new AppRewardsDistributor(
            protocol.token,
            ADMIN_MSIG,
            address(0)
        ); // Factory set later
        console2.log(
            "   AppRewardsDistributor deployed at:",
            address(
                protocol
                    .appRewardsDistributor
            )
        ...
        if (routerAddress != address(0))
        {
            ...
            console2.log(
                "   AppFactory skipped (no router configured)"
            );
        }
        ...
        protocol.tournamentFactory =
        new TournamentFactory(
            ADMIN_MSIG, INITIAL_TREASURY
        );
        console2.log(
    ...
    function _configurePermissions(
        ...
        protocol.xp.grantRole(
            protocol.xp.XP_OPERATOR_ROLE(
            ),
            address(protocol.funding)
        );
    ...
    function _logDeployment(
        ...
        console2.log(
            "4. Grant additional roles as needed"
        );
        console2.log(
            "=================="
        );
