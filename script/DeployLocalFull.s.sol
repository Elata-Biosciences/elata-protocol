...
import { LotPool } from
    "../src/governance/LotPool.sol";
import { RewardsDistributor } from
    "../src/rewards/RewardsDistributor.sol";
import { ElataGovernor } from
    "../src/governance/ElataGovernor.sol";
import { ElataTimelock } from
    "../src/governance/ElataTimelock.sol";
...
contract MockUniswapV2Router {
    ...
    {
        amounts =
            new uint256[](path.length);
        amounts[0] = amountIn;
        amounts[path.length - 1] =
            amountIn; // 1:1 for simplicity
        return amounts;
    }
...
contract DeployLocalFull is Script {
    ...
    uint256 public constant
        INITIAL_MINT = 10_000_000 ether; // 10M ELTA
    uint256 public constant MAX_SUPPLY =
        77_000_000 ether; // 77M ELTA total cap
    uint256 public constant
        TIMELOCK_DELAY = 1 hours; // Shorter for local testing
    uint256 public constant
        TEST_ACCOUNT_ELTA =
            100_000 ether; // 100K ELTA per test account
        ...
        )
    {
        ...
        console2.log(
            "  ELATA PROTOCOL - LOCAL DEVELOPMENT DEPLOYMENT"
        );
        console2.log(
            "=================================================\n"
        );
        ...
        console2.log(
            "       ELTA Token deployed at:",
            address(result.token)
        );

        // ===== STEP 2: Deploy XP System =====
        console2.log(
            "[2/10] Deploying ElataXP..."
        );
        result.xp =
            new ElataXP(result.deployer);
        console2.log(
            "       ElataXP deployed at:",
            address(result.xp)
        );
        ...
        console2.log(
            "[3/10] Deploying VeELTA Staking..."
        );
        result.staking = new VeELTA(
            result.token,
            result.deployer
        );
        console2.log(
        ...
        console2.log(
            "[4/10] Deploying Governance (Timelock + Governor)..."
        );
        result.timelock =
        _deployTimelock(result.deployer);
        result.governor = new ElataGovernor(
            result.token
        );
        console2.log(
        ...
        console2.log(
            "       Governor deployed at:",
            address(result.governor)
        );

        // ===== STEP 5: Deploy Funding System =====
        console2.log(
            "[5/10] Deploying LotPool Funding..."
        );
        result.funding = new LotPool(
            result.token,
            result.xp,
            result.deployer
        );
        console2.log(
            "       LotPool deployed at:",
            address(result.funding)
        );

        // ===== STEP 6: Deploy Rewards System =====
        console2.log(
            "[6/10] Deploying Rewards Distributor..."
        );
        ...
        console2.log(
            ...
            address(result.rewards)
        );

        // ===== STEP 7: Deploy Mock Uniswap =====
        console2.log(
            "[7/10] Deploying Mock Uniswap (Factory + Router)..."
        );
        result.uniFactory =
            new MockUniswapV2Factory();
        result.uniRouter = new MockUniswapV2Router(
            address(result.uniFactory)
        );
        console2.log(
            "       Mock Uniswap Factory deployed at:",
            address(result.uniFactory)
        );
        console2.log(
            "       Mock Uniswap Router deployed at:",
            address(result.uniRouter)
        );

        // ===== STEP 8: Deploy App Launch Framework =====
        console2.log(
            "[8/10] Deploying App Factories..."
        );
        ...
        console2.log(
            ...
            address(
                result.tournamentFactory
            )
        );

        // ===== STEP 9: Configure Permissions =====
        console2.log(
            "[9/10] Configuring Permissions..."
        );
        ...
        console2.log(
            "[10/10] Setting up Test Accounts..."
        );
        result.testAccounts =
        _setupTestAccounts(result.token);
        console2.log(
    ...
    function _logDeploymentSummary(
        ...
        console2.log(
            "App Factory:             ",
            address(result.appFactory)
        );
        console2.log(
            "App Factory Views:       ",
            address(
                result.appFactoryViews
            )
        );
        console2.log(
            "App Module Factory:      ",
        ...
        console2.log(
            "1. Run seed script: npm run dev:seed"
        );
        console2.log(
            "2. Start frontend: npm run dev:frontend"
        );
        console2.log(
            "3. Addresses saved to: deployments/local.json"
        );
        console2.log(
            "=================================================\n"
        );
