...
contract AppRewardsDistributor is
    AccessControl
{
    ...
    constructor(
        ...
        _grantRole(
            GOVERNANCE_ROLE, _governance
        );
        _grantRole(
            FACTORY_ROLE, _factory
        );
    }
     ...
     * @param vault Address of AppStakingVault
     */
    function registerApp(address vault)
        external
     ...
     * @param vault Vault address
     */
    function removeApp(address vault)
        external
        onlyRole(GOVERNANCE_ROLE)
    {
        if (!isVault[vault]) {
    ...
    function claim(
        ...
        AppEpoch[] storage vaultEpochs =
            epochs[vault];
        if (vaultEpochs.length == 0) {
            revert NoEpochs();
        }

        uint256 fromEpoch = userCursor[msg
            .sender][vault];
        ...
        for (
            ...
            totalClaim += (
                epoch.amount * userStake
            ) / epoch.totalStaked;
        }

        userCursor[msg.sender][vault] =
            endEpoch;
    ...
    function claimToken(
        ...
        if (
            vaultTokenEpochs.length == 0
        ) return; // No epochs to claim

        uint256 fromEpoch = tokenCursor[msg
            .sender][vault][token];
        uint256 endEpoch = toEpoch
            > vaultTokenEpochs.length
            ? vaultTokenEpochs.length
            : toEpoch;
        ...
        tokenCursor[msg.sender][vault][token]
        = endEpoch;
