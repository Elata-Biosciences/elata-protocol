...
contract LpLocker {
     ...
     */
    function lockLp(uint256 amount)
        external
    {
        require(
    ...
    function claim() external {
        ...
        require(
            balance > 0,
            "No LP tokens to claim"
        );

        bool success = IUniswapV2Pair(
            lpToken
        ).transfer(beneficiary, balance);
        if (!success) {
            revert TransferFailed();
        }

        emit LpClaimed(
            appId, beneficiary, balance
        );
    }
