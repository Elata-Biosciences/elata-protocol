    ...
    AccessControl
{
    ...
    uint16 public constant
        MAX_TRANSFER_FEE_BPS = 200; // 2% max
        ...
        ERC20Permit(name_)
    {
        ...
        transferFeeExempt[address(this)]
        = true;
        ...
        )
    {
        ...
        isExempt = transferFeeExempt[msg
            .sender];
    }
    ...
    function _update(
        ...
        if (
            from == address(0)
                || to == address(0)
                || transferFeeExempt[from]
                || transferFeeExempt[to]
                || transferFeeBps == 0
        ) {
            super._update(
                from, to, amount
        ...
        uint256 veFee =
            (fee * 1500) / 10_000;
        uint256 treasuryFee =
            fee - appFee - veFee; // Avoid rounding issues
