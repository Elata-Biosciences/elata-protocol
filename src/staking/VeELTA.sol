    ...
    AccessControl
{
    ...
    uint64 public constant MIN_LOCK =
        7 days;
    uint64 public constant MAX_LOCK =
        730 days; // 2 years

    /// @notice Boost values (1e18 = 1x)
    uint256 public constant BOOST_MIN =
        1e18; // 1.00x at MIN_LOCK
    uint256 public constant BOOST_MAX =
        2e18; // 2.00x at MAX_LOCK
    ...
    function lock(
        ...
        if (
            unlockTime
                <= block.timestamp
                    + MIN_LOCK
        ) revert Errors.LockTooShort();
        if (
            unlockTime
                > block.timestamp + MAX_LOCK
        ) revert Errors.LockTooLong();
        ...
        uint256 duration = unlockTime
            - uint64(block.timestamp);
        uint256 boost =
            _calculateBoost(duration);
        uint256 veAmount =
            (amount * boost) / 1e18;
    ...
    function increaseAmount(
        ...
        uint256 remainingTime = userLock
            .unlockTime
            - uint64(block.timestamp);
        uint256 boost = _calculateBoost(
            remainingTime
        );

        // Calculate old and new voting power
        uint256 oldVeAmount = (
            uint256(userLock.principal)
                * boost
        ) / 1e18;
        uint256 newPrincipal = uint256(
            userLock.principal
        ) + amount;
        uint256 newVeAmount = (
            newPrincipal * boost
        ...
        locks[msg.sender].principal =
            uint128(newPrincipal);
