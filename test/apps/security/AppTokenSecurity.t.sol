 ...
 */
contract AppTokenSecurityTest is
    Test
{
    ...
    function test_Security_OnlyMinterCanMint(
    ) public {
        vm.expectRevert();
        vm.prank(attacker);
    ...
    function test_Security_OnlyAdminCanFinalize(
    ) public {
        vm.expectRevert();
        vm.prank(attacker);
    ...
    function test_Security_OnlyCreatorCanUpdateMetadata(
    ) public {
        vm.expectRevert(
            AppToken
                .OnlyCreator
    ...
    function test_Security_OnlyAdminCanRevokeMinter(
    ) public {
        vm.expectRevert();
        vm.prank(attacker);
    ...
    function test_Security_CannotMintAboveCap(
    ) public {
        vm.prank(admin);
    ...
    function test_Security_CapEnforcedIncrementally(
    ) public {
        vm.startPrank(admin);

        // Mint 90% of cap
        token.mint(
            user1, (MAX_SUPPLY * 9) / 10
        );
    ...
    function testFuzz_Security_SupplyNeverExceedsCap(
        ...
        amount1 = bound(
            amount1, 1, MAX_SUPPLY
        );
        amount2 = bound(
            amount2, 1, MAX_SUPPLY
        );
    ...
    function test_Security_CannotMintAfterFinalize(
    ) public {
        vm.prank(admin);
        token.finalizeMinting();
    ...
    function test_Security_FinalizationIrreversible(
    ) public {
        vm.prank(admin);
        token.finalizeMinting();

        assertTrue(
            token.mintingFinalized()
        );
    ...
    function test_Security_BurnReducesSupply(
    ) public {
        vm.prank(admin);
        ...
        assertEq(
            token.totalSupply(),
            supplyBefore - 500 ether
        );
    }

    function test_Security_BurnFromRequiresApproval(
    ) public {
        vm.prank(admin);
        token.mint(user1, 1000 ether);
    ...
    function test_Security_BurnFromWorksWithApproval(
    ) public {
        vm.prank(admin);
        token.mint(user1, 1000 ether);
    ...
    function test_Security_PermitNonceIncreases(
    ) public {
    ...
    function test_Security_MinterRoleRevokable(
    ) public {
        assertTrue(
            token.hasRole(
                token.MINTER_ROLE(),
    ...
    function test_Security_CannotGrantMinterRoleUnauthorized(
    ) public {
    ...
    function test_Security_TransferFeeApplied(
    ) public {
        vm.prank(admin);
        token.mint(user1, 1000 ether);
    ...
    function test_Security_OwnerFunctionReturnsCreator(
    ) public {
        assertEq(token.owner(), creator);
    }

    function test_Security_OwnerDoesNotChangeOnMint(
    ) public {
        vm.prank(admin);
        token.mint(user1, 1000 ether);
    ...
    function test_Security_MetadataUpdateOnlyByCreator(
    ) public {
        vm.prank(creator);
        token.updateMetadata(
            "Real", "Real", "Real"
    ...
    function test_Security_CanBurnAfterFinalize(
    ) public {
        vm.prank(admin);
        token.mint(user1, 1000 ether);
    ...
    function test_Security_CanTransferAfterFinalize(
    ) public {
        vm.prank(admin);
        token.mint(user1, 1000 ether);
    ...
    function test_Security_CanApproveAfterFinalize(
    ) public {
        vm.prank(admin);
        token.mint(user1, 1000 ether);
