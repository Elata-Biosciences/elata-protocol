 ...
 */
contract TournamentSecurityTest is
    Test
{
    ...
    uint256 public constant ENTRY_FEE =
        100 ether;
    ...
    function test_Security_CannotClaimWithInvalidProof(
    ) public {
        // Setup
        vm.prank(user1);
        appToken.approve(
        ...
        vm.prank(attacker);
        tournament.claim(
            wrongProof, 100 ether
        );
    }

    function test_Security_CannotClaimWithModifiedAmount(
    ) public {
        vm.prank(user1);
        appToken.approve(
            address(tournament),
    ...
    function test_Security_CannotClaimTwice(
    ) public {
        vm.prank(user1);
        appToken.approve(
            address(tournament),
    ...
    function test_Security_FeeCapEnforced(
    ) public {
        vm.expectRevert(
            Tournament
                .FeesTooHigh
    ...
    function test_Security_FeeCalculationCorrect(
    ) public {
        // Enter tournament
        vm.prank(user1);
        appToken.approve(
        ...
        uint256 expectedProtocol =
            (totalPool * 250) / 10000; // 2.5%
        uint256 expectedBurn =
            (totalPool * 100) / 10000; // 1%
        uint256 expectedNet = totalPool
            - expectedProtocol
            - expectedBurn;
    ...
    function test_Security_CannotChangeFeesAfterFinalize(
    ) public {
        vm.prank(owner);
        tournament.finalize(bytes32(0));
    ...
    function test_Security_CannotEnterTwice(
    ) public {
        vm.startPrank(user1);
        appToken.approve(
            address(tournament),
    ...
    function test_Security_CannotEnterAfterFinalize(
    ) public {
        vm.prank(owner);
        tournament.finalize(bytes32(0));
        ...
        (bool canEnter, uint8 reason) =
        tournament.checkEntryEligibility(
            user1
        );
    ...
    function test_Security_CannotFinalizeTwice(
    ) public {
        vm.startPrank(owner);
        tournament.finalize(
            bytes32(uint256(1))
    ...
    function test_Security_CannotEnterOutsideWindow(
    ) public {
    ...
    function test_Security_PoolAccounting(
    ) public {
        uint256 users = 10;
    ...
    function testFuzz_Security_FeeCalculationCorrect(
        ...
        uint256 expectedProtocol = (
            poolAmount * protocolBps
        ) / 10000;
        uint256 expectedBurn = (
            poolAmount * burnBps
        ) / 10000;
        uint256 expectedNet = poolAmount
            - expectedProtocol
            - expectedBurn;
    ...
    function test_Security_OnlyOwnerCanSetFees(
    ) public {
        vm.expectRevert();
        vm.prank(attacker);
        tournament.setFees(300, 200);
    ...
    function test_Security_OnlyOwnerCanSetWindow(
    ) public {
        vm.expectRevert();
        vm.prank(attacker);
    ...
    function test_Security_OnlyOwnerCanSetEntryFee(
    ) public {
        vm.expectRevert();
        vm.prank(attacker);
    ...
    function test_Security_ZeroEntryFeeTournament(
    ) public {
        ...
        assertTrue(
            freeTourn.entered(user1)
        );
    }

    function test_Security_HighFeesEnforced(
    ) public {
        // Max 15% total fees
        vm.prank(owner);
    ...
    function test_Security_FinalizeWithEmptyPool(
    ) public {
    ...
    function test_Security_ReentrancyProtection_Enter(
    ) public {
    ...
    function test_Security_BurnActuallyHappens(
    ) public {
        vm.prank(user1);
        appToken.approve(
            address(tournament),
        ...
        uint256 expectedBurnAfterFee =
            (expectedBurn * 99) / 100;

        // Verify burn sink received tokens (after transfer fee)
        assertEq(
            appToken.balanceOf(burnSink),
            expectedBurnAfterFee
        );
    ...
    function test_Security_ViewFunctionsConsistent(
    ) public {
        Tournament fuzzTourn = new Tournament(
            address(appToken),
            owner,
