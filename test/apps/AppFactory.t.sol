...
contract MockRewardsDistributor is
    IRewardsDistributor
{
...
contract AppFactoryTest is Test {
    ...
    MockAppFeeRouter public
        mockFeeRouter;
    MockAppRewardsDistributor public
        mockAppRewards;
    MockRewardsDistributor public
        mockRewards;
    ...
    function test_Deployment() public {
        assertEq(
            address(factory.ELTA()),
            address(elta)
        );
        assertEq(
            address(factory.router()),
            mockRouter
        );
        assertEq(
    ...
    function test_CreateApp() public {
        uint256 totalCost = factory
            .creationFee()
            + factory.seedElta(); // 10 + 100 = 110 ELTA
        ...
        AppToken token =
            AppToken(app.token);
        assertEq(
            token.name(), "NeuroGame"
        );
        assertEq(
            token.symbol(), "NEURO"
        );
        assertEq(
            token.appCreator(), creator
        );
        assertEq(
            token.totalSupply(),
            factory.defaultSupply()
        ...
        uint256[] memory creatorApps =
        views.getCreatorApps(creator);
    ...
    function test_RevertWhen_CreateAppPaused(
    ) public {
        vm.prank(admin);
    ...
    function test_RevertWhen_CreateAppInsufficientFunds(
    ) public {
    ...
    function test_ParametersAreImmutable(
    ) public view {
        assertEq(
            factory.seedElta(),
            100 ether
