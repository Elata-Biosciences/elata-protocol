...
contract AppModuleFactoryTest is
    Test
{
    ...
    function test_Deployment() public {
        ...
        assertEq(
            factory.createFeeELTA(), 0
        );
    }

    function test_DeploymentWithZeroELTA(
    ) public {
        AppModuleFactory noFeeFactory =
        ...
        public
    {
        ...
        (
            ...
            address storedStaking,
            address storedEpochs
        ) = factory.modulesByApp(
            address(appToken)
        );
        assertEq(
            storedAccess, access1155
        );
    ...
    function test_EpochRewardsDeployment(
    ) public {
        vm.prank(appCreator);
        (,, address epochs) = factory
            .deployModules(
            address(appToken),
            "https://test/"
        );
    ...
    function test_DeployModulesWithELTAFee(
    ) public {
        // Set fee
        vm.prank(factoryOwner);
    ...
    function test_RevertWhen_DeployModulesNotTokenOwner(
    ) public {
        vm.expectRevert(
            AppModuleFactory
                .NotTokenOwner
    ...
    function test_RevertWhen_DeployModulesTwice(
    ) public {
        vm.prank(appCreator);
        factory.deployModules(
            address(appToken),
    ...
    function test_RevertWhen_DeployModulesWithoutELTAApproval(
    ) public {
        vm.prank(factoryOwner);
        factory.setCreateFee(CREATE_FEE);
    ...
    function test_SetTreasury()
        public
    {
        address newTreasury =
            makeAddr("newTreasury");
    ...
    function test_RevertWhen_SetTreasuryUnauthorized(
    ) public {
        vm.expectRevert();
        vm.prank(user1);
        factory.setTreasury(
    ...
    function test_RevertWhen_SetCreateFeeUnauthorized(
    ) public {
        vm.expectRevert();
        vm.prank(user1);
    ...
    function test_MultipleAppsDeployModules(
    ) public {
        ...
        (
            address storedAccess1,
            address storedStake1,
            address storedEpochs1
        ) = factory.modulesByApp(
            address(appToken)
        );
        ...
        (
            address storedAccess2,
            address storedStake2,
            address storedEpochs2
        ) = factory.modulesByApp(
            address(appToken2)
        );
    ...
    function test_DeployModulesAndConfigureItems(
    ) public {
        // Deploy modules
        vm.prank(appCreator);
        (address access1155,,) = factory
            .deployModules(
            address(appToken),
            "https://metadata.test/"
        );
    ...
    function test_DeployModulesAndStake(
    ) public {
        // Mint tokens to user
        vm.prank(admin);
        ...
        vm.startPrank(user1);
        appToken.approve(
            stakingVault, 500 ether
        );
        AppStakingVault(stakingVault)
            .stake(500 ether);
        vm.stopPrank();
    ...
    function test_DeployModulesWithZeroFee(
    ) public {
    ...
    function test_DeployModulesWithFactoryELTADisabled(
    ) public {
        // Factory with ELTA disabled
        AppModuleFactory noEltaFactory =
        new AppModuleFactory(
            address(0),
            factoryOwner,
            treasury
        ...
        vm.prank(factoryOwner);
        noEltaFactory.setCreateFee(
            100 ether
        ); // Set fee (but ELTA is disabled)

        // Should work without ELTA transfer
        vm.prank(appCreator);
        noEltaFactory.deployModules(
            address(appToken),
            "https://metadata.test/"
        );
    }
