...
contract LpLockerTest is Test {
    ...
    uint256 public constant
        LOCK_DURATION = 365 days * 2; // 2 years
    ...
    function test_RevertWhen_DeploymentInvalidParams(
    ) public {
    ...
    function test_LockLp() public {
        ...
        assertEq(
            locker.getLockedBalance(),
            lockAmount
        );
    }

    function test_RevertWhen_LockZeroAmount(
    ) public {
    ...
    function test_TimeUntilUnlock()
        public
    {
        uint256 timeRemaining =
            locker.timeUntilUnlock();
        assertEq(
            timeRemaining, LOCK_DURATION
        );
    ...
    function test_Claim() public {
        ...
        assertEq(
            locker.getLockedBalance(), 0
        );
        assertTrue(locker.claimed());
    }

    function test_RevertWhen_ClaimTooEarly(
    ) public {
        uint256 lockAmount = 100 ether;
        lpToken.transfer(
            address(locker), lockAmount
    ...
    function test_RevertWhen_ClaimUnauthorized(
    ) public {
        uint256 lockAmount = 100 ether;
        lpToken.transfer(
            address(locker), lockAmount
    ...
    function test_GetLockedBalance()
        public
    {
        assertEq(
            locker.getLockedBalance(), 0
    ...
    function testFuzz_LockAndClaim(
        ...
        timeOffset = bound(
            timeOffset, 0, 365 days * 5
        ); // Up to 5 years

        // Mint LP tokens to this contract
        lpToken.mint(
            address(this), lockAmount
        );
    ...
    function test_MultipleDepositsBeforeClaim(
    ) public {
        ...
        lpToken.transfer(
            address(locker), amount1
        );
        locker.lockLp(amount1);

        lpToken.transfer(
            address(locker), amount2
        );
        locker.lockLp(amount2);

        lpToken.transfer(
            address(locker), amount3
        );
    ...
    function test_LockDurationEdgeCases(
    ) public {
