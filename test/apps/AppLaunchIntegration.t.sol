...
import { AppFactory } from
    "../../src/apps/AppFactory.sol";
import { AppFactoryViews } from
    "../../src/apps/AppFactoryViews.sol";
...
import { AppBondingCurve } from
    "../../src/apps/AppBondingCurve.sol";
import { LpLocker } from
    "../../src/apps/LpLocker.sol";
import { IUniswapV2Router02 } from
    "../../src/interfaces/IUniswapV2Router02.sol";
import { IAppFeeRouter } from
    "../../src/interfaces/IAppFeeRouter.sol";
import { IAppRewardsDistributor } from
    "../../src/interfaces/IAppRewardsDistributor.sol";
import {
    MockAppFeeRouter,
 ...
 */
contract AppLaunchIntegrationTest is
    Test
{
    ELTA public elta;
    ...
    function setUp() public {
        ...
        MockAppFeeRouter mockFeeRouter =
            new MockAppFeeRouter();
        MockAppRewardsDistributor
        ...
        elta.transfer(
            creator1, 10_000 ether
        );
        elta.transfer(
            creator2, 10_000 ether
        );
        elta.transfer(
            investor1, 50_000 ether
        );
        elta.transfer(
            investor2, 50_000 ether
        );
        elta.transfer(
            investor3, 50_000 ether
        );
        vm.stopPrank();
        ...
        mockXP.setBalance(
            investor1, 1000 ether
        );
        mockXP.setBalance(
            investor2, 1000 ether
        );
        mockXP.setBalance(
            investor3, 1000 ether
        );
    }

    function _setupMockUniswap()
        internal
    {
        vm.mockCall(
            mockRouter,
    ...
    function test_CompleteAppLaunchLifecycle(
    ) public {
    ...
    {
        uint256 totalCost = factory
            .creationFee()
            + factory.seedElta();

        vm.startPrank(creator1);
        elta.approve(
            address(factory), totalCost
        );
        ...
        uint256 defaultSupply =
            factory.defaultSupply();
        uint256 creatorStaked =
            defaultSupply / 2; // 50% auto-staked to creator
        uint256 curveSupply =
        defaultSupply - creatorStaked; // 50% to curve

        assertEq(
            token.name(), "NeuroRacing"
        );
        ...
        AppStakingVault vault =
            AppStakingVault(app.vault);
        assertEq(
            vault.balanceOf(creator1),
            creatorStaked
        ); // Creator has 50% staked
        assertEq(
            token.balanceOf(app.curve),
            curveSupply
        ); // Curve has 50%
    ...
    function _testBondingCurvePhase(
        ...
        for (
            ...
            uint256 tokensOut = curve
                .buy(amount, expectedTokens);
            vm.stopPrank();
        ...
        uint256 defaultSupply =
            factory.defaultSupply();
        uint256 curveSupply =
            defaultSupply / 2; // V2: 50% to curve, 50% auto-staked

        assertGt(
            eltaReserve,
            factory.seedElta()
        ); // Should have more than seed
        assertEq(
            tokenReserve,
            curveSupply
                - totalTokensPurchased
        ...
        public
    {
        ...
        uint256[] memory creator1Apps =
        views.getCreatorApps(creator1);
        uint256[] memory creator2Apps =
        views.getCreatorApps(creator2);

        assertEq(creator1Apps.length, 2);
        assertEq(creator2Apps.length, 1);
        assertEq(
            creator1Apps[0], appId1
        );
        assertEq(
            creator1Apps[1], appId2
        );
        assertEq(
            creator2Apps[0], appId3
        );

        // Test concurrent trading on different curves
        AppFactory.App memory app1 =
            factory.getApp(appId1);
        AppFactory.App memory app2 =
            factory.getApp(appId2);

        AppBondingCurve curve1 =
            AppBondingCurve(app1.curve);
        AppBondingCurve curve2 =
            AppBondingCurve(app2.curve);
        ...
        uint256 tokens1 =
            curve1.buy(500 ether, 0);
        uint256 tokens2 =
            curve2.buy(500 ether, 0);
        ...
        console2.log(
            "[OK] Multiple app launches working correctly"
        );
    }

    function test_AppLaunchEconomics()
        public
    {
        // Test the economic flows of app launches

        uint256 treasuryBefore =
            elta.balanceOf(treasury);
        ...
        uint256 treasuryAfterCreation =
            elta.balanceOf(treasury);

        // Verify creation fee collected
        assertEq(
            treasuryAfterCreation
                - treasuryBefore,
            factory.creationFee()
        );
        ...
        console2.log(
            "[OK] Economic flows verified"
        );
    }

    function test_FactoryParameterUpdates(
    ) public {
        ...
        assertEq(
            curve.targetRaisedElta(),
            factory.targetRaisedElta()
        );
        assertEq(
            token.maxSupply(),
            factory.defaultSupply()
        ...
        console2.log(
            "[OK] Parameter updates working correctly"
        );
    }

    function test_AppRegistryFunctionality(
    ) public {
        ...
        uint256[] memory
            graduatedAppsList =
                views.getGraduatedApps();
        assertEq(
            graduatedAppsList.length, 0
        );

        console2.log(
            "[OK] Registry functionality verified"
        );
    }

    function test_SecurityMechanisms()
        public
    {
        ...
        console2.log(
            "[OK] Security mechanisms verified"
        );
    }

    function test_GasOptimization()
        public
    {
    ...
    function testFuzz_AppLaunchScenarios(
        ...
        purchaseAmount = bound(
            purchaseAmount,
            1 ether,
            (targetAmount - seedAmount)
                / 2
        );
        ...
        uint256 creatorStaked =
            supply / 2; // 50% auto-staked
        uint256 curveSupply =
            supply - creatorStaked;

        assertEq(
            token.maxSupply(), supply
        );
        assertEq(
            vault.balanceOf(creator1),
            creatorStaked
