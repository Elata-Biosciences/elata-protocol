...
contract AppBondingCurveTest is Test {
    ...
    address public governance =
        makeAddr("governance");
    address public mockAppRewards =
        makeAddr("mockAppRewards");
    address public mockRewards =
        makeAddr("mockRewards");

    address public mockRouter =
        makeAddr("mockRouter");

    uint256 public constant SEED_ELTA =
        100 ether;
    uint256 public constant
        TARGET_RAISED = 42_000 ether;
    uint256 public constant
        TOKEN_SUPPLY =
            1_000_000_000 ether;
    ...
    function setUp() public {
        ...
        vm.prank(treasury);
        elta.transfer(
            address(curve), SEED_ELTA
        );

        vm.prank(factory);
        curve.initializeCurve(
            SEED_ELTA, TOKEN_SUPPLY
        );
    ...
    function test_GetTokensOut()
        public
    {
        // Test price calculation
        uint256 tokensOut = curve
            .getTokensOut(1000 ether);
        assertGt(tokensOut, 0);

        // More ELTA should get fewer tokens (increasing price)
        uint256 tokensOut2 = curve
            .getTokensOut(2000 ether);
        assertLt(
            tokensOut2, tokensOut * 2
        ); // Non-linear due to curve
    }
    ...
    function test_Buy() public {
        ...
        elta.approve(
            address(curve), eltaIn
        );

        uint256 tokensBefore =
            appToken.balanceOf(buyer1);
        uint256 tokensOut = curve.buy(
    ...
    function test_BuyWithSlippageProtection(
    ) public {
        uint256 eltaIn = 1000 ether;
        uint256 minTokensOut = curve
            .getTokensOut(eltaIn) + 1; // Set too high
    ...
    function test_RevertWhen_BuyAfterGraduation(
    ) public {
    ...
    function testFuzz_BuyTokens(
        ...
        uint256 expectedTokens = curve
            .getTokensOut(eltaAmount);

        vm.startPrank(buyer1);
        elta.approve(
            address(curve), eltaAmount
        );
        uint256 tokensOut =
            curve.buy(eltaAmount, 0);
        vm.stopPrank();
