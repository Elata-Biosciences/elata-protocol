 ...
 */
contract AppModulesIntegrationTest is
    Test
{
    ...
    uint256 public constant MAX_SUPPLY =
        1_000_000_000 ether;
    uint256 public constant CREATE_FEE =
        50 ether;

    event FeatureAccessGranted(
    ...
    function setUp() public {
        ...
        vm.prank(factoryOwner);
        elta.mint(
            appCreator, 1000 ether
        );

        vm.prank(appCreator);
        elta.approve(
            address(factory), CREATE_FEE
        );
        ...
        (
            address accessAddr,
            address vaultAddr,
        ) = factory.deployModules(
            address(appToken),
            "https://metadata.neuropong.game/"
        );

        access =
            AppAccess1155(accessAddr);
        vault =
    ...
    function test_Integration_CompleteAppLaunch(
    ) public {
        ...
        assertEq(
            access.owner(), appCreator
        );
        assertEq(
            vault.owner(), appCreator
        );
        ...
        // Verify registry
        (
            address storedAccess,
            address storedVault,
            address storedEpochs
        ) = factory.modulesByApp(
            address(appToken)
        );
        assertEq(
            storedAccess,
        ...
        assertEq(
            storedVault, address(vault)
        );
        assertTrue(
            storedEpochs != address(0)
        );
    }

    function test_Integration_SeasonPassPurchaseAndStaking(
    ) public {
        ...
        uint256 initialSupply =
            appToken.totalSupply();

        // Player1 purchases season pass
        vm.startPrank(player1);
        appToken.approve(
            address(access), 100 ether
        );
        access.purchase(
            1, 1, keccak256("season_1")
        );
        vm.stopPrank();

        // Verify purchase
        assertEq(
            access.balanceOf(player1, 1),
            1
        );
    ...
    function test_Integration_FeatureGatingFullFlow(
    ) public {
        ...
        bool hasAccess = access
            .checkFeatureAccess(
            player1, premiumFeature, 0
        );
    ...
    function test_Integration_TournamentFullWorkflow(
    ) public {
        // Configure premium tournament (requires stake)
        bytes32 tourneyAccess =
        keccak256("tournament_access");
        vm.prank(appCreator);
        access.setFeatureGate(
            tourneyAccess,
            AppAccess1155.FeatureGate({
        ...
        uint256 expectedProtocol =
            (20 ether * 250) / 10000;
        uint256 expectedProtocolAfterFee =
        (expectedProtocol * 99) / 100;
        assertEq(
            appToken.balanceOf(treasury),
            expectedProtocolAfterFee
        );
    }

    function test_Integration_SeasonalRewardsFullFlow(
    ) public {
    ...
    function test_Integration_MultipleAppsCoexist(
    ) public {
        // Deploy second app
        ...
        AppAccess1155 access2 =
            AppAccess1155(access2Addr);
        AppStakingVault vault2 =
            AppStakingVault(vault2Addr);
    ...
    function test_Integration_StakeGateUnstakeFlow(
    ) public {
    ...
    function test_Integration_EconomicLoop(
    ) public {
        // Track token flow through the ecosystem
        uint256 initialCreatorBalance =
        appToken.balanceOf(appCreator);
        uint256 initialSupply =
            appToken.totalSupply();
    ...
    function test_Integration_TieredAccessSystem(
    ) public {
        // Setup tiered system
        vm.startPrank(appCreator);
        ...
        // Buy gold + stake more
        appToken.approve(
            address(access), 500 ether
        );
        access.purchase(
            3, 1, bytes32(0)
        );
        appToken.approve(
            address(vault), 400 ether
        );
        ...
        assertEq(
            access.balanceOf(player1, 2),
            1
        );
        assertEq(
            access.balanceOf(player1, 3),
            1
        );
        assertEq(
            vault.stakedOf(player1),
    ...
    function test_Integration_CompleteSeason(
    ) public {
    ...
    function test_Sanity_TokenSupplyNeverIncreases(
    ) public {
        uint256 initialSupply =
            appToken.totalSupply();
        ...
        assertEq(
            appToken.totalSupply(),
            initialSupply - 300 ether
        );
    }

    function test_Sanity_StakingDoesNotChangeSupply(
    ) public {
        uint256 initialSupply =
            appToken.totalSupply();
    ...
    function test_Sanity_AllModulesOwnedByCreator(
    ) public {
        ...
        assertEq(
            rewards.owner(), appCreator
        );
    }

    function test_Sanity_ELTAFlowsToTreasury(
    ) public {
        ...
        assertEq(
            elta.balanceOf(treasury),
            treasuryBefore + CREATE_FEE
        );
    }

    function test_Sanity_ProtocolFeesAccumulate(
    ) public {
        uint256 treasuryBefore =
            appToken.balanceOf(treasury);
        ...
        uint256 expectedFee =
            (20 ether * 250) / 10000;
        uint256 expectedFeeAfterTransfer =
            (expectedFee * 99) / 100;
        assertEq(
            appToken.balanceOf(treasury),
            treasuryBefore
                + expectedFeeAfterTransfer
        );
    }
}
