...
contract AppFeeRouterTest is Test {
        ...
        view
    {
        ...
        assertEq(
            feeRouter.governance(),
            governance
        );
        assertEq(
            feeRouter.feeBps(), 100
        ); // 1%
        assertEq(
            feeRouter.MAX_FEE_BPS(), 500
        ); // 5%
    }

    function test_TakeAndForwardFee()
        public
    {
        uint256 grossAmount = 1000 ether;
        uint256 expectedFee =
            (grossAmount * 100) / 10_000; // 1% = 10 ether
        ...
        assertEq(
            elta.balanceOf(trader),
            10_000 ether - expectedFee
        );
    }

    function test_TakeAndForwardFee_SmallAmount(
    ) public {
        uint256 traderBalanceBefore =
            elta.balanceOf(trader);
        uint256 grossAmount = 9 ether;
        uint256 expectedFee =
            (grossAmount * 100) / 10_000; // 0.09 ELTA
    ...
    function test_SetFeeBps() public {
        ...
        assertEq(
            feeRouter.feeBps(), 250
        );

        // Verify new fee rate works
        uint256 grossAmount = 1000 ether;
        uint256 expectedFee =
            (grossAmount * 250) / 10_000; // 2.5% = 25 ether
    ...
    function test_SetFeeBps_RevertIfNotGovernance(
    ) public {
        vm.prank(trader);
        vm.expectRevert(
            AppFeeRouter
    ...
    function test_SetFeeBps_RevertIfTooHigh(
    ) public {
        vm.prank(governance);
        vm.expectRevert(
            AppFeeRouter
    ...
    function test_SetFeeBps_MaxAllowed()
        public
    {
        vm.prank(governance);
        ...
        public
    {
        ...
        feeRouter.setFeeBps(200);
        assertEq(
            feeRouter.feeBps(), 200
        );
    }

    function test_TransferGovernance_RevertZeroAddress(
    ) public {
        vm.prank(governance);
    ...
    function test_CalculateFee()
        public
        view
    {
        uint256 amount = 1000 ether;
        uint256 fee = feeRouter
            .calculateFee(amount);
        assertEq(fee, 10 ether); // 1% of 1000 = 10

        assertEq(
            feeRouter.calculateFee(0), 0
        );
        assertEq(
