 ...
 */
contract AppLaunchSecurityTest is
    Test
{
    ELTA public elta;
    ...
    function test_Critical_UnauthorizedAppCreation(
    ) public {
    ...
    function test_Critical_FactoryAccessControl(
    ) public {
    ...
    function test_Critical_TokenSupplyProtection(
    ) public {
        // Create app
    ...
    function test_Critical_BondingCurveReentrancy(
    ) public {
        // Create app
        uint256 totalCost = factory
            .creationFee()
        ...
        MockElataXP mockXP = MockElataXP(
            address(factory.elataXP())
        );
        mockXP.setBalance(
            creator, 1000 ether
        );
    ...
    function test_Critical_LpLockerSecurity(
    ) public {
    ...
    function test_Critical_ProtocolFeeCollection(
    ) public {
        ...
        assertEq(
            elta.balanceOf(attacker),
            5_000 ether
        ); // Original amount, no fees
    }

    function test_Critical_PauseMechanism(
    ) public {
    ...
    function test_Critical_TokenMetadataProtection(
    ) public {
    ...
    function test_Critical_BondingCurveProtection(
    ) public {
        ...
        uint256 actualTokens = curve.buy(
            eltaIn, expectedTokens
        );
        vm.stopPrank();
    ...
    function test_Critical_ZeroAddressProtection(
    ) public {
        // All contracts should reject zero addresses

        // Deploy mocks first (before expectRevert)
        MockAppFeeRouter mockFee =
            new MockAppFeeRouter();
        MockAppRewardsDistributor
            mockRewards = new MockAppRewardsDistributor(
            );

        MockElataXP mockXP =
            new MockElataXP();
        MockRewardsDistributor
            mockRewards2 = new MockRewardsDistributor(
            );
    ...
    function test_Critical_ParameterValidation(
    ) public {
        ...
        assertGt(
            factory.defaultSupply(), 0
        );
        assertGt(
            factory.lpLockDuration(), 0
        );
    }

    function test_Critical_AppTokenTransferability(
    ) public {
        ...
        assertEq(
            actualRemaining,
            tokensOut - transferAmount
        );
    }

    function test_Critical_CreationStakeRequirement(
    ) public {
        // Creators must stake ELTA to create apps

        uint256 requiredStake = factory
            .creationFee()
            + factory.seedElta();
        ...
        assertEq(
            elta.balanceOf(creator),
            10_000 ether - requiredStake
        );
    }
}
