...
contract VeELTATest is Test {
        ...
        view
    {
        ...
        assertEq(
            veElta.symbol(), "veELTA"
        );
        assertEq(
            veElta.MIN_LOCK(), 7 days
        );
        assertEq(
            veElta.MAX_LOCK(), 730 days
        );
    }

    function test_Lock_MaxDuration()
        public
    {
        uint256 amount = 1000 ether;
        uint64 unlockTime = uint64(
        ...
        assertEq(
            veElta.balanceOf(user1),
            amount * 2
        );
    }

    function test_Lock_MinDuration()
        public
    {
        uint256 amount = 1000 ether;
        uint64 unlockTime = uint64(
    ...
    function test_Lock_RevertIfTooShort(
    ) public {
        uint64 tooShort = uint64(
            block.timestamp + 6 days
        );

        vm.prank(user1);
        vm.expectRevert(
            Errors.LockTooShort.selector
        );
        veElta.lock(
            1000 ether, tooShort
        );
    ...
    function test_Lock_RevertIfTooLong()
        public
    {
        uint64 tooLong = uint64(
            block.timestamp + 731 days
        );

        vm.prank(user1);
        vm.expectRevert(
            Errors.LockTooLong.selector
        );
        veElta.lock(1000 ether, tooLong);
    }

    function test_Lock_RevertIfAlreadyExists(
    ) public {
        vm.startPrank(user1);
        veElta.lock(
            1000 ether,
    ...
    function test_ExtendLock() public {
        ...
        uint64 initialUnlock = uint64(
            block.timestamp + 365 days
        );
        vm.prank(user1);
        veElta.lock(
            1000 ether, initialUnlock
        );
        ...
        public
    {
        ...
        assertEq(
            veElta.balanceOf(user2), 0
        );
    }

    function test_Snapshots_GetPastVotes(
    ) public {
        vm.roll(100);
